...
10. **New `model_config` field `ndt_edge_width` sets where the response-time admissibility floor sits for models with trial-to-trial non-decision-time variability** (#XXXX; stacked on #1292). The guard that floors the log-likelihood below the model's fastest admissible response time places that edge at `t - st`, which is exact only for a non-decision-time kernel with compact support of half-width `st` (the convention every LAN `*_st` network follows). Kernels with unbounded support — e.g. `Normal(t, st)`, where `st` is a standard deviation — carry real density below `t - st`, and flooring there discards likelihood the model genuinely assigns. `ndt_edge_width` moves the floor to `t - ndt_edge_width * st`. A likelihood entry in a registered model's config may also declare it, so a supported model ships the correct edge as its own default; because it is declared per likelihood, a blackbox exact likelihood and its LAN approximation for the same model can carry different edges. `None` (the default) means 1.0, byte-identical to the previous behaviour; the field has no effect on models without `st`, nor on `full_ddm`, whose blackbox likelihood reads `st` as a full width and already returns zero density below `t - st/2`, above this floor. Unbounded kernels should set 3.0, the kernel's practical 3-sigma edge.

11. **`ddm_normalt` is now a supported model** (#YYYY; requires #1292 and the `ndt_edge_width` entry above). The DDM with Normal trial-to-trial variability in non-decision time is registered for `loglik_kind="approx_differentiable"`, backed by the `ddm_normalt.onnx` LAN, with parameters `v, a, z, t, st`. The trial non-decision time is `Normal(t, st)`, so `st` is the kernel standard deviation and the kernel support is unbounded; the entry therefore ships `ndt_edge_width = 3.0` in its likelihood config, placing the admissibility floor at the practical 3-sigma edge `t - 3 * st` instead of the compact-kernel edge `t - st` (measured: floor at `t - st` gives rhat 3.39 / ESS 4; at `t - 3 * st`, rhat 1.010 / ESS 697 with 0% divergences). Selecting the model string is enough — the correct edge is not something users have to remember. Its `bounds` are the network's training box, not modelling choices, which is why they are narrower than the family norm: `t` starts at 0.25 and `z` is confined to (0.3, 0.7), against `t` from 0.0 and `z` in (0.1, 0.9) for plain `ddm`. Two caveats users should act on. First, the sibling registration `ddm_st` declares the same `st` bounds with a different meaning — there `st` is a uniform half-width, whose standard deviation is `st / sqrt(3)` — so an equal `st` across the two models is not an equal amount of non-decision-time variability. Second, although `st = 1e-3` is the genuine lower edge of the training box, it is a defect corner of this network: below `st = 0.01` the network places its maximum on the bound where the exact likelihood has an interior maximum, with the gradient pointing into the bound, so chains trap at `st ≈ 0.002` and per-subject recovery degrades while rhat and ESS stay clean. Constrain `st` to `(0.01, 0.25)` for inference (measured at `st = 0.05`: correlation with truth for `v` +0.24 → +0.97, coverage 6/12 → 11/12).

12. **`ddm_st` is now a supported model** (#XXXX; requires #1292). The DDM with uniform trial-to-trial variability in non-decision time is registered for `loglik_kind="approx_differentiable"`, backed by the `ddm_st.onnx` LAN, with parameters `v, a, z, t, st`. The trial non-decision time is `Uniform(t - st, t + st)`, so `st` is the kernel half-width and the kernel standard deviation is `st / sqrt(3)`. Its `bounds` are the network's training box, not modelling choices, which is why they are narrower than the family norm: `t` starts at 0.25 and `z` is confined to (0.3, 0.7), against `t` from 0.0 and `z` in (0.1, 0.9) for plain `ddm`. Sampling outside that box asks the network to extrapolate. Correct inference also requires the `st`-aware admissibility floor (#1292): without it the guard floors the `[t - st, t]` band, where this kernel puts real density. A separate registration, `ddm_normalt`, declares the same `st` bounds with a different meaning — there `st` is the standard deviation of an unbounded `Normal(t, st)` kernel — so an equal `st` across the two models is not an equal amount of non-decision-time variability.
